...
shared ({ caller = creator }) actor class AxonService() = this {
  ...
  */
  public query func cycles<system>() : async Nat {
    Cycles.balance();
  };
  ...
  public shared ({ caller }) func upgradeProxy() : async [Result.Result<Bool, Text>] {
    ...
    for (thisAxon in SB.vals(state_current.axons)) {
      ...
      try {
        ...
        //send more cycles
        //Cycles.add<system>(2_000_000_000_000);// the ledger arghive needs 2T cycles
        Debug.print("done");
      ...
      for (b in Map.vals(p.ballots)) {
        if (b.principal == caller) {
          if (Option.isSome(b.vote)) {
            ...
            // Add reward to voter
            let votingReward = 10;
            ignore await mint(request.axonId, caller, votingReward);
          };
